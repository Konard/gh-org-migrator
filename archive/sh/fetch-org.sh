...
if [ -f .env ]; then
    ...
    echo ".env file not found! Please create one with GITHUB_ACCESS_TOKEN and ORGANIZATION."
    exit 1
fi

# Check if GITHUB_ACCESS_TOKEN and ORGANIZATION are set
if [ -z "$GITHUB_ACCESS_TOKEN" ] || [ -z "$ORGANIZATION" ]; then
    echo "GITHUB_ACCESS_TOKEN and ORGANIZATION must be set in the .env file."
    exit 1
fi
